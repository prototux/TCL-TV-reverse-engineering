...
if [ "$CMD" = "P2P-GROUP-STARTED" ]; then
    GIFNAME=$3
    if [ "$4" = "GO" ]; then
	kill_daemon udhcpc /data/run/udhcpc-$GIFNAME.pid
	ifconfig $GIFNAME 192.168.42.1 up
	udhcpd /etc/wifi/udhcpd-p2p.conf
    fi
    if [ "$4" = "client" ]; then
	kill_daemon udhcpc /data/run/udhcpc-$GIFNAME.pid
	kill_daemon udhcpd /data/run/udhcpd-$GIFNAME.pid
	udhcpc -i $GIFNAME -p /data/run/udhcpc-$GIFNAME.pid \
